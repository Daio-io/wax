...
extract_core() {
  ...
  jq --arg generated_at "$GENERATED_AT" --arg source_scan "$SOURCE_SCAN" --argjson limits "$LIMITS_JSON" '
    ...
    def symbol_rollups_block:
      {
        design_system: (
          symbol_rollup(all_usage_sites; "resolved"; "registry")
          + symbol_rollup(all_usage_sites; "candidate"; "registry")
        )
        | group_by(.symbol)
        | map({symbol: .[0].symbol, count: (map(.count) | add)})
        | sort_by(-.count, .symbol),
  ...
  jq --slurpfile baseline "$baseline_file" --slurpfile current "$current_scan" '
    ...
    def repo_block($scan):
      if $scan.repo_summary? != null then
        ...
        reduce ($scan.languages[] | .counts.raw_invocations) as $raw (
          {total: 0, resolved: 0, local: 0, candidate: 0, unresolved: 0};
          {
            total: (.total + ($raw.total // 0)),
            resolved: (.resolved + ($raw.resolved // 0)),
            local: (.local + ($raw.local // 0)),
            candidate: (.candidate + ($raw.candidate // 0)),
            unresolved: (.unresolved + ($raw.unresolved // 0))
          }
        ) as $raw
        | {
            invocation_adoption_ratio: (
