...
#[test]
fn subprocess_echoes_scan_request_and_returns_scan_facts() {
...
#[test]
fn subprocess_maps_timeout_wire_error() {
    let temp_dir = TestDir::new("wire-timeout");
    let script_path = temp_dir.path().join("mock-pack.sh");
    write_script(
        &script_path,
        &format!(
...
#[test]
fn subprocess_rejects_unsupported_api_version() {
    let temp_dir = TestDir::new("unsupported-api-version");
    let script_path = temp_dir.path().join("mock-pack.sh");
    write_script(
        &script_path,
        &format!(
...
#[test]
fn subprocess_reports_failed_process_when_stdout_is_not_wire_response() {
    let temp_dir = TestDir::new("failed-process-garbage-stdout");
    let script_path = temp_dir.path().join("mock-pack.sh");
    write_script(
        &script_path,
        r#"#!/bin/sh
...
#[test]
fn subprocess_maps_elapsed_timeout() {
    let temp_dir = TestDir::new("elapsed-timeout");
    let script_path = temp_dir.path().join("mock-pack.sh");
    write_script(
        &script_path,
        r#"#!/bin/sh
