...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let cli = Cli::parse();
    require_stdio(cli.stdio, "wax-lang-swift");
    Ok(serve_stdio(&SwiftWireHandler(SwiftLanguage::new()))?)
}
...
fn run_stdio_with_reader<R: BufRead, W: Write>(
    ...
    serve_one(reader, writer, &SwiftWireHandler(SwiftLanguage::new()))
}

struct SwiftWireHandler(SwiftLanguage);

impl WirePackHandler for SwiftWireHandler {
    fn language_id(&self) -> LanguageId {
        pack_language_id("swift")
    }
