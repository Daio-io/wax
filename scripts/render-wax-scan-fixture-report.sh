...
if boundary_rows:
    ...
    for item in boundary_rows:
        raw_boundary = item.get("raw_invocations") or {}
        ...
        boundary_table_rows.append(
            ...
            f'<td class="num">{num(raw_boundary.get("resolved", 0))}</td>'
            f'<td class="num">{num(raw_boundary.get("local", 0))}</td>'
            f'<td class="num">{num(raw_boundary.get("candidate", 0))}</td>'
            f'<td class="num">{num(raw_boundary.get("unresolved", 0))}</td>'
...
if not boundary_rows:
    rendered = remove_section(rendered, "Source boundaries")
