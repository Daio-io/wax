...
fn new_parser() -> Result<tree_sitter::Parser, TreeSitterScanError> {
    ...
    parser.set_language(&swift_language()).map_err(|reason| {
        TreeSitterScanError::ParserInitFailed {
...
fn swift_language() -> tree_sitter::Language {
    ...
    unsafe { tree_sitter::Language::from_raw(language_ptr as *const tree_sitter::ffi::TSLanguage) }
}
